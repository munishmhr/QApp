define([
	...
	'views/indexLeftPane', 
	'views/enterQuestionIndexLeftPane', 
	'views/reviewQuestionIndexLeftPane', 
		...
		var Index = Backbone.View.extend({
			...
			*/
			render: function () {
				...
				this.collection.each(function (technology) {
					var indexLeftHtml = (new indexLeftPane({
						model: technology
					})).render().el;
			...
			*/
			showquestion: function (events) {
				...
				var quesCollection = new QuestionCollection({
					subtechnology: $(events.target).html()
				});
				var that = this;
				$('#side-right-pane-question-spaceId').empty();
				quesCollection.fetch({
					...
					success: function () {
						...
						quesCollection.each(function (quest) {
							var reviewLeftHtml = (new reveiwQuestion({
								model: quest
							})).render().el;
			...
			*/
			enterQuestion: function () {
                console.log("index.js::enterQuestion");
                $("#li_enter_question").addClass("active");
                $("#li_review_question").removeClass("active");
			...
			*/
			...
			reviewQuestion: function (event) {
                $("#li_review_question").addClass("active");
                $("#li_enter_question").removeClass("active");
